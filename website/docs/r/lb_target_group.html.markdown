...
* `preserve_client_ip` - (Optional) Boolean to indicate whether client IP preservation is enabled or disabled. The default is disabled if the target group type is IP address and the target group protocol is TCP or TLS. Otherwise, the default is enabled. Client IP preservation cannot be disabled for UDP and TCP_UDP target groups.
